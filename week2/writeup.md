...
Prompt:
```
TODO:  Update week2/tests/test_extract.py to add unit tests for extract_action_items_llm(). Mock the Ollama chat function with pytest monkeypatch so the tests do not depend on a real local model. Cover bullet lists, keyword-prefixed lines, empty input, duplicate items, and invalid JSON responses.
